...
class QuotaTracker:
    ...
    def add_files(self, n: int, *, archive_files_so_far: int = 0) -> None:
        """Account for ``n`` files produced by one extraction step.

        Enforces the per-archive file-count ceiling against the count this
        extraction produced. Raises :class:`SafetyLimitExceeded` on breach.
        This is the enforcement path for ``--max-files`` (SEC-2): a single
        extraction that emits more than ``max_files_per_archive`` entries (an
        archive-of-a-million-tiny-files bomb) trips the ceiling.
        """
        if n <= 0:
            return
        with self._lock:
            observed = archive_files_so_far + n
            if observed > self.max_files_per_archive:
                raise SafetyLimitExceeded(
                    "max_files_per_archive",
                    self.max_files_per_archive,
                    observed,
