"""
================================================================================
manual_install_windows.py -- Auto-install handler subsystem for Windows tools
                              that have no winget Package ID
================================================================================
...
empty hint, indicating "no winget package -- manual install required". Through
v0.4.3, those tools were skipped entirely by `--install`, leaving the user with
a partial install batch and a list of names to track down manually. Per the
v0.4.4 scope decisions confirmed by the user:
...
- The libyal toolset (16 binaries: ewf*, vmdk*, vhdi*, qcow*, fs{apfs,ntfs,
  ...
  third-party sources. Lesson L37 captures this constraint.
...
Version
-------
v0.4.4 -- initial implementation (this module did not exist in v0.4.3 or earlier).
================================================================================
"""
...
#: Per the v0.4.4 user-confirmed Q1 answer: machine-scope, never user scope.
...
_USER_AGENT: str = "re-unpacker/0.4.4 (manual-install)"
...
def _http_download(url: str, dest: Path, *, logger: logging.Logger) -> None:
...
def _add_install_dir_to_system_path(*, logger: logging.Logger) -> bool:
    ...
    v0.4.7 (lesson L44): Windows shows a modal "PATH env variable too big"
    ...
    5. Continue gracefully -- re-unpacker's Pass 0 well-known lookup (added
       in v0.4.6) finds our installed tools regardless of PATH state, so
...
#   - returns succeeded / failed / skipped per Q4 contract
#
...
_OUT_OF_SCOPE: dict[str, str] = {
    "signtool":  "Windows SDK (user has SDK installed; out-of-scope per v0.4.4 Q3)",
    "apksigner": "Android SDK (out-of-scope per v0.4.4 Q3)",
    "apktool":   "Android SDK (out-of-scope per v0.4.4 Q3)",
}
...
def _dispatch_handler(
    ...
    *,
    ...
    if name_l in ("ewfinfo", "ewfexport"):
        # v0.4.6 (efficiency fix): pass BOTH ewfinfo and ewfexport in one call.
        ...
        # 14MB of redundant transfer per the v0.4.5 field log. Returning
    ...
    # v0.4.6 (lesson L42): asset_name_hint MUST be 'win-x64' not 'x64'.
    ...
    *,
    ...
    for tool_name in tool_names:
        ...
        try:
            ...
            # Handler crashed unexpectedly. Log + continue per Q4.
        ...
        for r in results:
            ...
            # v0.4.6 (lesson L39 fulfillment): log per-tool result so the
    ...
    # v0.4.6: also log the summary so the log file is self-contained for
