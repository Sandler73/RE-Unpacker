...
#
# v0.3.1 (ISS-004): expanded from 33 entries (covering only v0.1.x tools) to
...
_VERSION_PROBES_LINUX: dict[str, Sequence[str]] = {
    ...
    # v0.3.1: Disk image conversion
    "qemu-img": ("--version",),
    # v0.3.1: libyal -- uniformly accept -V (uppercase, single-dash)
    "vmdkmount": ("-V",),
    "vmdkinfo": ("-V",),
    "qcowmount": ("-V",),
    "qcowinfo": ("-V",),
    ...
    # v0.3.2: Subsystem C classifiers
...
# -----------------------------------------------------------------------------
# Windows-platform version probe args (v0.4.0)
# -----------------------------------------------------------------------------
...
# -----------------------------------------------------------------------------
# Platform-resolved alias (v0.4.0)
# -----------------------------------------------------------------------------
...
# safely exec'd as a probe (v0.4.2; lesson L32).
# -----------------------------------------------------------------------------
...
#     with an os.killpg AttributeError (fixed in v0.4.2 via
...
_NO_EXEC_PROBE_TOOLS: frozenset[str] = frozenset({
    "msiexec",
    # v0.4.7 (cosmetic): plistutil with no args waits for stdin; with
...
class ToolRegistry:
    ...
    def probe(self, name: str, *, logger: logging.Logger | None = None) -> Tool:
        ...
        # v0.4.2 (lesson L33): use platform_compat.which_tool which adds
        ...
        if path is not None:
            # v0.3.1 (ISS-004): every available tool is now probed. The
            ...
            # uninformative "(version: unknown)" display the v0.1.x / v0.2.0
            ...
            # v0.4.2 (lesson L32): some Windows tools have no console
        ...
        if logger is not None:
            if tool.available:
                ...
                # v0.4.2 (lesson L34): empty package_hint means "manual
...
def format_tools_check_report(registry: ToolRegistry) -> str:
    ...
    # v0.4.2 (lesson L34, Ryan field report): filter out tools whose
    ...
    if missing:
        # v0.4.2 (lesson L34): Platform-aware footer. On Linux, list the
        ...
        # winget Package IDs in an apt-flavored command (the v0.4.0/v0.4.1
