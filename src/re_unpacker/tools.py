...
#
# expanded from 33 entries (covering only v0.1.x tools) to
...
class ToolRegistry:
    ...
    def probe(self, name: str, *, logger: logging.Logger | None = None) -> Tool:
        ...
        # use platform_compat.which_tool which adds
        ...
        if path is not None:
            # every available tool is now probed. The
            ...
            # some Windows tools have no console
        ...
        if logger is not None:
            if tool.available:
                ...
                # empty package_hint means "manual
...
def format_tools_check_report(registry: ToolRegistry) -> str:
    ...
    if missing:
        # Platform-aware footer. On Linux, list the
