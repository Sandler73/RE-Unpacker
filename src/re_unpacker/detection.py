...
class FileKind(str, enum.Enum):
    ...
    # ---- v0.2.0 additions: extended unpack formats ----
...
EXTRACTABLE_KINDS: frozenset[FileKind] = frozenset({
    ...
    # ---- v0.3.0 additions: extraction-depth (extractable only) ----
    # APK is extractable via apktool (and as a ZIP).
    FileKind.APK,
    # PDF is now extractable (qpdf for structure, pdfdetach for attachments).
    # We add it here because v0.3.0 introduces a real extractor for it.
    FileKind.PDF,
...
def _refine_with_file_description(
    ...
    if base_kind == FileKind.ZIP:
        # APK detection (v0.3.0): Android package archives are ZIP files at
...
def _compound_ext(path: Path) -> str:
    ...
    for suffix in (
        ...
        ".tar.lz4", ".tar.lzo",
        ".tar.lz",  # v0.2.0: lzip-wrapped tar
    ):
