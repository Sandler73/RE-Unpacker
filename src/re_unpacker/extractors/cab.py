...
Notes
-----
- Two independent paths cover the format: ``cabextract`` where present, and
  the built-in ``expand.exe`` on Windows. Neither is required for the other
  to work, so a host with only one still extracts cabinets.
- Multi-part cabinets that continue into a sibling file only extract fully
  when the continuation files are present in the same directory as the
  source; otherwise the trailing member is truncated and the tool reports it.
