...
``is_supported()`` selecting at runtime:
...
- **DebSevenZipExtractor** (Windows; new in v0.4.0): three-stage 7-Zip
...
The orchestrator picks the highest-priority is_supported extractor; on
...
class DebExtractor(Extractor):
    ...
    so ``is_supported()`` returns False on Windows automatically.
    """
...
class DebSevenZipExtractor(Extractor):
    ...
    # priority dpkg-deb / ar+tar paths; we override is_supported() below
