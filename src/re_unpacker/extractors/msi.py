...
- **Windows**: ``msiexec /a "<file>" /qn TARGETDIR=<dir>``
...
Notes
-----
- The Windows path uses an administrative install (``msiexec /a ... /qn``),
  which expands the file tree without running install custom actions. That
  distinction matters for malware triage: a normal install would execute
  attacker-controlled code with the analyst's privileges.
- The Linux path (``msitools``) reads the MSI's internal CAB streams
  directly, so it never executes anything, but it also cannot resolve
  properties that only a real installer session would compute.
