...
Version
-------
Added in 0.4.0 as part of the Windows-tandem release.
"""
...
def is_macos() -> bool:
    ...
    Note: re-unpacker doesn't officially support macOS as of v0.4.0;
...
def which_tool(name: str) -> Path | None:
    ...
    - On Windows (v0.4.2; lesson L33), if PATH lookup misses, fall back
    ...
    # v0.4.2 (lesson L33): Windows fallback to well-known install dirs.
    if _IS_WINDOWS:
...
# -----------------------------------------------------------------------------
# Windows well-known install directories (v0.4.2; lesson L33)
# -----------------------------------------------------------------------------
...
def _windows_well_known_lookup(name: str) -> Path | None:
    ...
    v0.4.3 (lesson L36): expanded coverage for winget's portable-package
    ...
    # ---- Pass 0 (v0.4.6, lesson L41): re-unpacker's own manual-install dir.
    # Tools auto-installed by re-unpacker --install on Windows land at
    # C:\Program Files\re-unpacker\bin\ (per the v0.4.4 manual_install_windows
    ...
    # ---- Pass 6 (v0.4.6, lesson L43): Python Scripts directories.
...
def _re_unpacker_install_dirs() -> list[Path]:
    ...
    (see ``manual_install_windows.py``). Per the v0.4.4 user-confirmed Q1
...
def refresh_path_from_registry() -> bool:
    ...
    Added in v0.4.2 (lesson L33) after a Windows 11 Pro field bug where
    winget-installed tools weren't detected in the same process that
...
__all__ = [
    ...
    "long_path_supported",
    "normalize_long_path",
    # PATH refresh after winget install (v0.4.2)
