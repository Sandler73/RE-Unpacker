...
def which_tool(name: str) -> Path | None:
    ...
    - On Windows, if PATH lookup misses, fall back
    ...
    # Windows fallback to well-known install dirs.
    if _IS_WINDOWS:
...
# -----------------------------------------------------------------------------
# Windows well-known install directories
# -----------------------------------------------------------------------------
...
def _windows_well_known_lookup(name: str) -> Path | None:
    ...
    expanded coverage for winget's portable-package
    ...
    # ---- Pass 0: RE-Unpacker's own manual-install dir.
    ...
    # ---- Pass 6: Python Scripts directories.
