...
Description
-----------
Three distinct verification mechanisms exist for ``.deb`` files; v0.3.2
runs all that are available against any DEB kind file.
...
class DebsumsVerifier(Verifier):
    ...
    NOTE (v0.3.2): debsums fundamentally operates on INSTALLED packages
    ...
    that have not been installed -- so for v0.3.2, this verifier is
    ...
    that doesn't require installation. Out of scope for v0.3.2.
    """
    ...
    def applies_to(self, file_entry: FileEntry) -> bool:
        # Disabled in v0.3.2 -- see class docstring.
        return False
