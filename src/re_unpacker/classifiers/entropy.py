...
Notes
-----
- The 7.5 bits-per-byte threshold is a heuristic, not proof: compressed
  archives and encrypted blobs both sit near 8.0. Treat a high value as a
  signal to inspect, not as a classification.
- This is the only classifier with no hard tool requirement, since the
  pure-Python fallback always works. It is therefore the pass most likely to
  run on a minimally provisioned host.
- The fallback is pure Python and holds the GIL, so it is the one enrichment
  path where a large file can measurably serialize parallel runs.
