...
def _build_parser() -> argparse.ArgumentParser:
    ...
    # conventional verbosity shortcuts.
...
def _validate_args(args: argparse.Namespace) -> Path | None:
    ...
    if args.input is None:
        ...
        # see .\re-unpacker.ps1 not just ./re-unpacker.
...
def _print_log_banner(file_path, mode: str) -> None:
    ...
    every non-extract mode shows this so the user can find
...
def _run_tools_check(args: argparse.Namespace) -> int:
    # tools-check now writes a default file log to
...
def _run_normal(
    ...
    logger = setup_logging(output_root, console_level=args.log_level)
    # when --log-file is passed in extract mode, attach an
...
def _run_install(args: argparse.Namespace) -> int:
    ...
    if not _ensure_root(mode_name="--install"):
        return 5
    # default file log to ~/.cache/re-unpacker/logs/.
    console_level = getattr(logging, args.log_level)
    logger, log_path = setup_dual_logging(
    ...
    # on Windows, several winget packages add
    ...
    if is_windows():
        # manual_install_windows is imported here
    ...
    # Non-zero exit code if any handler failed (signal to CI / scripts)
...
def _run_dry_run_install(args: argparse.Namespace) -> int:
    ...
    # platform-aware command rendering. On Windows
...
def _resolve_effective_log_level(args: argparse.Namespace) -> str:
    """Derive the effective log_level from args.
