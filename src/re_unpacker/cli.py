...
def _build_parser() -> argparse.ArgumentParser:
    ...
    # =========================================================================
    # v0.3.2: Enrichment flags (Subsystem B verification + Subsystem C classification)
    # =========================================================================
    enrich = p.add_argument_group("Enrichment (v0.3.2)")
    enrich.add_argument(
    ...
    # ---- v0.2.0: package-management modes (require root) ----
    mode.add_argument(
    ...
    # v0.3.1 (ISS-005): conventional verbosity shortcuts.
    ...
    # v0.3.1 (ISS-002): explicit log file path.
    log.add_argument(
...
def _validate_args(args: argparse.Namespace) -> Path | None:
    ...
    if args.input is None:
        # v0.4.1: include a literal usage example so the user knows
...
def _print_log_banner(file_path, mode: str) -> None:
    ...
    v0.3.1 (ISS-001): every non-extract mode shows this so the user can find
...
def _run_tools_check(args: argparse.Namespace) -> int:
    # v0.3.1 (ISS-001): tools-check now writes a default file log to
...
def _run_normal(
    ...
    logger = setup_logging(output_root, console_level=args.log_level)
    # v0.3.1 (ISS-002): when --log-file is passed in extract mode, attach an
    ...
    # v0.3.2: Configure enrichment registries from CLI flags.
...
# =============================================================================
# Package-management run modes (v0.2.0)
# =============================================================================
...
def _run_install(args: argparse.Namespace) -> int:
    ...
    if not _ensure_root(mode_name="--install"):
        return 5
    # v0.3.1 (ISS-001): default file log to ~/.cache/re-unpacker/logs/.
    console_level = getattr(logging, args.log_level)
    logger, log_path = setup_dual_logging(
    ...
    # v0.4.2 (lesson L33): on Windows, several winget packages add
    ...
    # v0.4.4: after the winget batch completes, run manual-install handlers
    ...
    if is_windows():
        # v0.4.5 (lesson L40): manual_install_windows is imported here
...
def _run_uninstall(args: argparse.Namespace) -> int:
    ...
    if not _ensure_root(mode_name="--uninstall"):
        return 5
    # v0.3.1 (ISS-001): default file log.
    console_level = getattr(logging, args.log_level)
    logger, log_path = setup_dual_logging(
...
def _run_dry_run_install(args: argparse.Namespace) -> int:
    ...
    # v0.4.2 (lesson L34): platform-aware command rendering. On Windows
...
def _resolve_effective_log_level(args: argparse.Namespace) -> str:
    """Derive the effective log_level from args (v0.3.1, ISS-005).
...
def main(argv: list[str] | None = None) -> int:
    ...
    # v0.3.1: resolve -v / -vv / -q into args.log_level so downstream code
