...
For only the Debian/Ubuntu/Kali ``apt-get`` backend is implemented.
...
class AptBackend(PackageManagerBackend):
    ...
    def is_package_available(self, package: str) -> bool:
        ...
        entry from poisoning a whole apt-get install batch (rc=100). added this after the libfsfat-utils incident: the libyal FAT mounter
        package is not in every Debian/Ubuntu release, and's all-or-
...
class WingetBackend(PackageManagerBackend):
    ...
    def _run_streaming(
        ...
        *,
        ...
        if rc != 0:
            # detect 0x80071130 Fast Cache error specifically.
    ...
    def _refresh_source(
        ...
        try:
            # Lesson L35: `winget source update` does NOT accept
    ...
    def install_packages(
        ...
        # refresh the source cache before installing. Mirrors
...
def is_root() -> bool:
    ...
    Windows (+): elevated administrator via the Win32 API
